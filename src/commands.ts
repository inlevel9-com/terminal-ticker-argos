...
export async function brief(ticker: string, f: Flags): Promise<void> {
  ...
  if (b) {
    out('')
    out(`${c.bold(ko ? 'AI 브리프' : 'AI brief')} ${c.dim(ko ? '(생성형 AI 작성)' : '(generated by AI)')}`)
  ...
  out(c.dim(ko ? '투자 권유가 아닌 참고 정보입니다. 투자 판단과 결과의 책임은 이용자에게 있습니다.' : 'For information only, not investment advice. Investment decisions are your own.'))
}
