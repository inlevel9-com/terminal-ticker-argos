...
function footer(tab: Tab, overlay: Overlay, lang: Lang): string {
  ...
  if (overlay === 'brief') return t(lang, 'j/k 스크롤 · b/Esc 닫기 · 투자 권유 아님', 'j/k scroll · b/Esc close · not investment advice')
...
function BriefPane({ ticker, lang, width, height, scroll, nonce }: { ticker: string; lang: Lang; width: number; height: number; scroll: number; nonce: number }) {
  ...
  return (
    <Box flexDirection="column">
      <Text><Text bold>{ticker}</Text> <Text color="gray">{t(lang, 'AI 브리프 · 생성형 AI 작성', 'AI brief · generated by AI')}{brief.updated_at ? ` · ${brief.updated_at.slice(0, 10)}` : ''}</Text></Text>
